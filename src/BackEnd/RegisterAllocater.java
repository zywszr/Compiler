...
import static IRClass.Inst.MOV;

public class RegisterAllocater {

    LineIR lineIR;
    ActAnalysiser actAnalysiser;
    ...
    ArrayList <PhyRegOprand> generalRegs;
    int regNum;

    public RegisterAllocater(LineIR _lineIR, int _tmpVarIdx) {
        lineIR = _lineIR;
    ...
    private boolean assignColors() {
        ...
        boolean ret = true;
        for (Oprand v : orderStack) {
            if (v instanceof PhyRegOprand) continue;
            HashSet <PhyRegOprand> choices = new HashSet<>(generalRegs);
            ...
            if (choices.isEmpty()) {
                ...
                PhyRegOprand p = null;
                for (PhyRegOprand reg : callerSave) {
